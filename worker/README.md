...
Client secret에서 파생한 키로 암호화하며, 세션을 편집기 브라우저의 localStorage에 보관합니다.
세션에는 8시간짜리 사용자 토큰과 갱신 토큰이 함께 들어 있습니다. 토큰이 만료되면 서버가
갱신 토큰으로 새 토큰을 받아 `X-Arete-Session` 헤더로 새 세션을 돌려주고, 편집기는 이를
저장합니다. 그래서 같은 브라우저에서는 최대 30일간 로그인이 유지됩니다.
로그아웃하거나 30일이 지나거나 갱신에 실패하면 다시 로그인합니다.
